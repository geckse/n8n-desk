...
- **`done` is the ONLY terminal event — every session MUST end with one, and every terminal `error` MUST be followed by a `done`.** The renderer keys "running" state (spinner, stop button, input lock) exclusively on `done`; `error` events are informational and may be non-terminal (e.g. `MCP_DISCOVERY_FAILED` — the run continues with local tools). Clearing running state on `error` hid the stop button while the agent kept streaming (the background-run bug). Guard the invoke `finally` with an identity check (`if (activeRunners.get(id) === active)`) so a replacement runner isn't orphaned by a stale IIFE. `agent:stop` always emits `done { cancelled }` — even with no registered runner — so a stop click can always unlock a desynced session; the renderer additionally re-marks a session as running on activity events (self-healing, `workflow_preview` excluded because detached preview fetches can land after completion).
